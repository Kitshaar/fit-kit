    ...
    return (
        <SectionWrapper id={'generate'} header={"generate your workout"} title={['it\'s', 'Huge', 'o\'clock']} >
            ...
            <div className='bg-slate-950 border border-solid border-blue-400 rounded-lg flex flex-col'>
                <button onClick={toggleModel} className='relative flex p-3 items-center justify-center'>
                    <p className='capitalize'>
                                ...
                                map((muscleGroup, muscleGroupIndex) => {
                                    return (
                                        <button onClick={() => {
                                            ...
                                            className={'hover:text-blue-400 px-4 duration-200 '
                                                + (muscles.includes(muscleGroup) ? 'text-blue-400' : '')} key={muscleGroupIndex}>
                ...
                {Object.keys(SCHEMES).map((schemes, schemesIndex) => {
                    return (
                        <button onClick={() => {
                            ...
                            + (schemes === goal ? 'border-blue-600' : 'border-blue-400')} key={schemesIndex}>
                            <p className='capitalize'>
                                {schemes.replace('_', ' ')}
                            </p>
                        </button>
